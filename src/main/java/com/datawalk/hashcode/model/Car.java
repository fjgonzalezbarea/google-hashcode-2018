...
public class Car {
    ...
    public void takeRide(int currentStep, Ride ride) {
        if (ridesTaken.isEmpty()) {
            stepToBeFree =
                currentStep + Math.abs(ride.startPositionX - ride.startPositionY) + ride.distance()
                    + 1;
        } else {
    ...
    public Long peso(Ride ride, int currentStep) {
        ...
        return Long.valueOf(
            ride.getFinishTime() - (ride.distance() + distanceLstRideToRide + 1 + currentStep));
    }
    ...
    public Pair<Integer, Integer> lastRideCoordinates() {
        return getLastRide().map(ride -> Pair.of(ride.finishPositionX, ride.finishPositionY))
            .orElse(Pair.of(0, 0));
    }

    public int score(int bonus) {
        return ridesTaken.stream().map(ride -> ride.distance() + bonus(ride, bonus))
            .reduce((a, b) -> a + b).orElse(0);
    }

    public String toString() {
        return "Car " + id + " took rides --> " + ridesTaken.stream()
            .map(ride -> ride.id.toString()).collect(Collectors.joining(",")) + "\n";
    }
